...
class Initializer {
 ...
 public:
  ...
  bool Initialize(const Frame &CurrentFrame, const vector<int> &vMatches12,
                  cv::Mat &Tcw1, cv::Mat &Tcw2, vector<cv::Point3f> &vP3D,
                  vector<bool> &vbTriangulated);

 private:
  bool initial_pair(
      /*const cv::Mat& cvR21, const cv::Mat& cvt21, */ cv::Mat &Twc1,
  ...
  // new implementation
  Frame m_refFrame;  // reference frame
  Frame m_curFrame;  // current frame
  
  Frame m_InitFrame;  // current frame
  Frame m_InitRefFrame;  // current frame
  
//   float Init_Scale;
//   Eigen::Vector3d Init_P0;
