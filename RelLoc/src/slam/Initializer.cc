...
namespace ORB_SLAM2
{
  ...
  bool Initializer::Initialize(const Frame &CurrentFrame, const vector<int> &vMatches12, cv::Mat &Tcw1,
                               cv::Mat &Tcw2, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
  {
    ...
    const float RH = SH / (SH + SF);
    // Try to reconstruct from homography or fundamental depending on the ratio (0.40-0.45)
    bool initial_state = false;
    if (RH > 0.4)
    {
      //       initial_state = ReconstructH(vbMatchesInliersH,H,mK,R21,t21,vP3D,vbTriangulated,1.0,50);
      initial_state = initial_pair(Tcw1, Tcw2, vP3D, vbTriangulated);
  ...
  bool Initializer::initial_pair(cv::Mat &Tcw1,
                                 cv::Mat &Tcw2,
                                 vector<cv::Point3f> &vP3D,
                                 vector<bool> &vbTriangulated)
  {
    if (mvMatches12.size() < 20)
      return false;
    if (mvKeys1.empty() || mvKeys2.empty())
      return false;

    vP3D = vector<cv::Point3f>(mvKeys1.size(), cv::Point3f(0, 0, 0));
    vbTriangulated = vector<bool>(mvKeys1.size(), false);

    Mat2X xI = Mat2X::Zero(2, mvMatches12.size());
    Mat2X xJ = Mat2X::Zero(2, mvMatches12.size());

    for (size_t i = 0; i < mvMatches12.size(); ++i)
    {
      Match mtIdx = mvMatches12[i];

      cv::KeyPoint kpt1 = mvKeys1[mtIdx.first];
      cv::KeyPoint kpt2 = mvKeys2[mtIdx.second];

      xI.col(i) = Vec2(kpt1.pt.x, kpt1.pt.y);
      xJ.col(i) = Vec2(kpt2.pt.x, kpt2.pt.y);
    }

    // set camera parameters
    camera::CameraIntrinsic camCalib(Converter::toMatrix3d(mK), m_refFrame.mImCols, m_refFrame.mImRows);

    std::vector<size_t> vecInliers;
    double errorMax = std::numeric_limits<double>::max();
    Mat4 Tij;
    bool bRet = PoseEstimate::relativePose(xI, xJ, camCalib, camCalib, Tij, vecInliers, errorMax);

    std::cout << "ErrorMax = " << errorMax << ", ";
    std::cout << "vecInliers = " << vecInliers.size() << "/" << mvMatches12.size() << std::endl;

    if (!bRet)
    {
      std::cout << "Failed to compute initial R|t !" << std::endl;
      return false;
    }

    // 帧2相对于世界坐标系的平移
    //     float scale;
    //     if(m_curFrame.GetAbsXY().flag == true)
    //     {
    //         scale = (m_curFrame.GetAbsXY().xyz-m_refFrame.GetAbsXY().xyz).norm();
    //     }else{
    //         scale = 1;
    //     }

    //     // 初始化对准
    //     Eigen::Matrix3d attitude = m_refFrame.GetAbsXY().qwxyz.matrix();
    //     Mat3 R1= attitude;
    //
    //     Eigen::Vector3d location = m_refFrame.GetAbsXY().xyz;
    //     Vec3 t1 = location;
    //     Vec3 C1 = t1;

    // transform to absolute coordinate system
    // 帧1相对于世界坐标系的旋转
    ...
    // 从帧1到帧2的变换矩阵
    Mat4 Ti = Mat4::Identity();
    Ti.block<3, 3>(0, 0) = R1;
    Ti.block<3, 1>(0, 3) = t1;
    Tij = Tij * Ti;
    // 帧2相对于世界坐标系的旋转
    Mat3 R2 = Tij.block<3, 3>(0, 0);
    Vec3 t2 = Tij.block<3, 1>(0, 3); // 为什么要乘以60？ //Vec3 t2 = Tij.block<3, 1>(0, 3);
                                     //      // 帧2的光心相在世界坐标系中的位置
    Vec3 C2 = -R2.transpose() * t2;

    std::cout << "C1: " << C1.transpose() << std::endl;
    std::cout << "C2: " << C2.transpose() << std::endl;

    Mat34 P1 = MultiViewCompute::getPFrmKRt(Converter::toMatrix3d(mK), R1, t1);
    Mat34 P2 = MultiViewCompute::getPFrmKRt(Converter::toMatrix3d(mK), R2, t2);
    ...
    for (size_t i = 0; i < mvMatches12.size(); ++i)
    {
      ...
      Vec2 xI(kpt1.pt.x, kpt1.pt.y);
      Vec2 xJ(kpt2.pt.x, kpt2.pt.y);

      Vec3 pt3 = MultiViewCompute::triangulate(P1, xI, P2, xJ);

      float cosParallax = MultiViewCompute::cmpCosParallax(C1, C2, pt3);
      float depth1 = MultiViewCompute::cmpDepth(R1, t1, pt3);
      float depth2 = MultiViewCompute::cmpDepth(R2, t2, pt3);
      float error1 = MultiViewCompute::cmpPrjError(P1, pt3, xI);
      float error2 = MultiViewCompute::cmpPrjError(P2, pt3, xJ);
      float depth = std::min(depth1, depth2);

      if (depth > 2.0 && cosParallax < 0.99998 && error2 < th && error1 < th)
      {
        // 重建出的三维点
        vP3D[mtIdx.first] = cv::Point3f(pt3[0], pt3[1], pt3[2]);
        // 三角化重建成功
        vbTriangulated[mtIdx.first] = true;
        // 视角
        vCosParallax.push_back(cosParallax);
      }
    }

    Mat4 matT1 = Mat4::Identity();
    Mat4 matT2 = Mat4::Identity();

    matT1.block<3, 3>(0, 0) = R1;
    matT1.block<3, 1>(0, 3) = t1;
    matT2.block<3, 3>(0, 0) = R2;
    matT2.block<3, 1>(0, 3) = t2;

    Tcw1 = Converter::toCvMat(matT1);
    Tcw2 = Converter::toCvMat(matT2);
                                 ...
                                 int minTriangulated)
  {
    ...
    int nMinGood = max(static_cast<int>(0.8 * nMatches), minTriangulated);
    ...
    {
      std::cout << "Not enough triangulated points reject initialization"
                << std::endl;
      return false;
    }
                                 ...
                                 int minTriangulated)
  {
        ...
        bestGood > minTriangulated && bestGood > 0.9 * N)
    {
      ...
      cout << m_refFrame.mnId << " " << m_curFrame.mnId << endl;
      cout << R21 << " " << t21 << endl;
      return true;
    }
