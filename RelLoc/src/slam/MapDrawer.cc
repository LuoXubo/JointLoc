...
void MapDrawer::SetCurrentGPSIMUPose(const cv::Mat &Tcw) {
  unique_lock<mutex> lock(mMutexCamera);
  mGPSIMUPose = Tcw.clone();
}
...
void MapDrawer::GetCurrentOpenGLGPSIMUMatrix(pangolin::OpenGlMatrix &M) {
  if (!mGPSIMUPose.empty()) {
    cv::Mat Rwc(3, 3, CV_32F);
    cv::Mat twc(3, 1, CV_32F);
    {
      unique_lock<mutex> lock(mMutexCamera);
      Rwc = mGPSIMUPose.rowRange(0, 3).colRange(0, 3).t();
      twc = -Rwc * mGPSIMUPose.rowRange(0, 3).col(3);
    }
