...
Frame::Frame(const Frame &frame)
  ...
  if (!frame.GetPose().Tcw().empty()) SetPose(frame.GetPose().Tcw());

  SetAbsXY(frame.GetAbsXY());
}
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp,
  ...
  ExtractORB(0, imGray);
  // std::cout << std::endl << "Time for ORB feature detection: " <<
  // double(clock() - timeStart) / CLOCKS_PER_SEC << " s" << std::endl;

  mNumOfKPts = mvKeys.size();
  ...
  mvuRight = vector<float>(mNumOfKPts, -1);
  mvDepth = vector<float>(mNumOfKPts, -1);

  mvpMapPoints = vector<MapPoint *>(mNumOfKPts, static_cast<MapPoint *>(NULL));
  mvbOutlier = vector<bool>(mNumOfKPts, false);
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  // std::cout << "ORB points : " << mvKeys.size() << std::endl;
...
void Frame::AssignFeaturesToGrid() {
  int nReserve = 0.5f * mNumOfKPts / (FRAME_GRID_COLS * FRAME_GRID_ROWS);
...
void Frame::ExtractORB(int flag, const cv::Mat &im) {
  // const float ratio = 0.87;
  // cv::Rect roi(0,0,im.cols, int(im.rows*ratio));
  cv::Rect roi;
  if (flag == 0)
    (*mpORBextractorLeft)(im, roi, mvKeys, mDescriptors);
  else
    (*mpORBextractorRight)(im, roi, mvKeysRight, mDescriptorsRight);
}
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit) {
  ...
  const cv::Mat Pc = mPose.Rcw() * P + mPose.tcw();
...
void Frame::UndistortKeyPoints() {
  ...
  cv::Mat mat(mNumOfKPts, 2, CV_32F);
  for (int i = 0; i < mNumOfKPts; i++) {
...
void Frame::ComputeStereoMatches() {
  mvuRight = vector<float>(mNumOfKPts, -1.0f);
  mvDepth = vector<float>(mNumOfKPts, -1.0f);
  ...
  vDistIdx.reserve(mNumOfKPts);

  for (int iL = 0; iL < mNumOfKPts; iL++) {
...
cv::Mat Frame::UnprojectStereo(const int &i) {
  ...
  if (z > 0) {
    ...
    return mPose.Rwc() * x3Dc + mPose.Ow();
  } else
    return cv::Mat();
}
