...
int ORBmatcher::SearchByBoW(KeyFrame *pKF, Frame &F,
  ...
  vpMapPointMatches =
      vector<MapPoint *>(F.mNumOfKPts, static_cast<MapPoint *>(NULL));
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2,
  ...
  for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++) {
    ...
    if (bestDist > (float)bestDist2 * mfNNratio) continue;
...
int ORBmatcher::SearchForTriangulation(
  ...
  int nmatches = 0;
  vector<bool> vbMatched2(pKF2->mNumOfKPts, false);
  vector<int> vMatches12(pKF1->mNumOfKPts, -1);
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame,
  ...
  const cv::Mat Rcw = CurrentFrame.GetPose().Rcw();
  const cv::Mat tcw = CurrentFrame.GetPose().tcw();

  const cv::Mat twc = -Rcw.t() * tcw;

  const cv::Mat Rlw = LastFrame.GetPose().Rcw();
  const cv::Mat tlw = LastFrame.GetPose().tcw();
  ...
  for (int i = 0; i < LastFrame.mNumOfKPts; i++) {
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, KeyFrame *pKF,
  ...
  const cv::Mat Rcw = CurrentFrame.GetPose().Rcw();
  const cv::Mat tcw = CurrentFrame.GetPose().tcw();
  const cv::Mat Ow = CurrentFrame.GetPose().Ow();
  ...
  for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
    ...
    if (pMP->isBad() || sAlreadyFound.count(pMP)) continue;
