...
float MapPoint::Depth(const cv::Mat& R, const cv::Mat& C) {
  unique_lock<mutex> lock(mMutexPos);
  cv::Mat v = R * (mWorldPos - C);
  return v.at<float>(2, 0);
}
...
void MapPoint::UpdateNormalAndDepth() {
  ...
  for (map<KeyFrame*, size_t>::iterator mit = observations.begin(),
       ...
       mit != mend; mit++) {
    KeyFrame* pKF = mit->first;
    cv::Mat Owi = pKF->GetPose().Ow();
    cv::Mat normali = mWorldPos - Owi;
    normal = normal + normali / cv::norm(normali);
    n++;
  }

  cv::Mat PC = Pos - pRefKF->GetPose().Ow();
  const float dist = cv::norm(PC);
...
float MapPoint::GetDistance() { return mfDistance; }
