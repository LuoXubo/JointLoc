...
Sim3Solver::Sim3Solver(KeyFrame *pKF1, KeyFrame *pKF2,
  ...
  cv::Mat Rcw1 = pKF1->GetPose().Rcw();
  cv::Mat tcw1 = pKF1->GetPose().tcw();
...
Sim3 Sim3Solver::ComputeSim3FromPointsets(cv::Mat &P1, cv::Mat &P2) {
  // Step 1: Centroid and relative coordinates
  cv::Mat Pr1(P1.size(),
              P1.type());  // Relative coordinates to centroid (set 1)
  cv::Mat Pr2(P2.size(),
  ...
  cv::Mat t12i(1, 3, P1.type());
  t12i = O1 - scale * R12i * O2;
  Sim3 sim(Converter::toMatrix3d(R12i), Converter::toVector3d(t12i), scale);
  return sim;
}
