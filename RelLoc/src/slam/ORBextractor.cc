...
void ORBextractor::operator()(InputArray _image, const Rect &_Roi,
  ...
  if (_Roi.width < 1 || _Roi.height < 1) {
    _descriptors = descriptors;
    return;
  }

  // filter with ROI
  vector<KeyPoint> vecKeyPtsTmp;
  vector<size_t> vecRowIdx;
  for (size_t i = 0; i < _keypoints.size(); ++i) {
    if (!_Roi.contains(_keypoints[i].pt)) continue;
    vecKeyPtsTmp.push_back(_keypoints[i]);
    vecRowIdx.push_back(i);
  }
  _keypoints = vecKeyPtsTmp;

  //_OutputArray descriptorsOut;
  Mat descriptorsTmp(vecRowIdx.size(), 32, CV_8U);
  for (size_t i = 0; i < vecRowIdx.size(); ++i)
    descriptorsTmp.row(i) = descriptors.row(vecRowIdx[i]);

  _descriptors = descriptorsTmp;
}
