...
void LocalMapping::Run() {
  ...
  while (1) {
    ...
    if (CheckNewKeyFrames()) {
      ...
      ProcessNewKeyFrame();
      // std::cout << "--LocalMapping:Preocess new key frame!!" << std::endl;
      ...
      if (!CheckNewKeyFrames() && !stopRequested()) {
        ...
        if (mpMap->KeyFramesInMap() > 2) {
          // Optimizer::PoseGraphGPSIMUCam(mpCurrentKeyFrame, mpMap);
          Optimizer::LocalBundleAdjustment(mpCurrentKeyFrame,
                                           mpTracker->mInitialFrame.mnId,
                                           &mbAbortBA, mpMap);
        }
...
void LocalMapping::CreateNewMapPoints() {
  ...
  cv::Mat Rcw1 = mpCurrentKeyFrame->GetPose().Rcw();
  cv::Mat Rwc1 = mpCurrentKeyFrame->GetPose().Rwc();
  cv::Mat tcw1 = mpCurrentKeyFrame->GetPose().tcw();
  cv::Mat Tcw1(3, 4, CV_32F);
  Rcw1.copyTo(Tcw1.colRange(0, 3));
  tcw1.copyTo(Tcw1.col(3));
  cv::Mat Ow1 = mpCurrentKeyFrame->GetPose().Ow();
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++) {
    ...
    // std::cout << "LocalMapping: vMatchedIndices: " << vMatchedIndices.size()
    // << std::endl;

    cv::Mat Rcw2 = pKF2->GetPose().Rcw();
    cv::Mat Rwc2 = Rcw2.t();
    cv::Mat tcw2 = pKF2->GetPose().tcw();
    ...
    float cosTh =
        Param::ANGLE_INTERSECTION_COS * (1.0 + log(100. / (float)nmatches));
    if (cosTh > Param::ANGLE_INTERSECTION_COS_UPPERBOUND)
      cosTh = Param::ANGLE_INTERSECTION_COS_UPPERBOUND;
    if (cosTh < Param::ANGLE_INTERSECTION_COS_LOWERBOUND)
      cosTh = Param::ANGLE_INTERSECTION_COS_LOWERBOUND;
    ...
    for (int ikp = 0; ikp < nmatches; ikp++) {
      ...
      if (cosParallaxRays < cosParallaxStereo && cosParallaxRays > 0 &&
          (bStereo1 || bStereo2 || cosParallaxRays < cosTh)) {
  ...
  // std::cout << "--LocalMapping: New added map points: " << nnew << std::endl;
}
...
cv::Mat LocalMapping::ComputeF12(KeyFrame *&pKF1, KeyFrame *&pKF2) {
  cv::Mat R1w = pKF1->GetPose().Rcw();
  cv::Mat t1w = pKF1->GetPose().tcw();
  cv::Mat R2w = pKF2->GetPose().Rcw();
  cv::Mat t2w = pKF2->GetPose().tcw();
