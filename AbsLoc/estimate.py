...
if __name__ == "__main__":
    ...
    # pre_region = regions[79] # 先验知识 Crater
    # pre_region = regions[61] # 先验知识 Gravel
    # pre_region = regions[107] # 先验知识 Mountain
    ...
    for cnt in range(num_uav):
        ...
        #
        # x,y,z,qw,qx,qy,qz,conf,idx = localize(tensorA, cnt, timestamp)
        # Begin to localize
        #
        start = time.time()
        best_score, best_match = 1.0, 0
        # for satellite in pre_region:  # satellite_folder: 2_250_0.png
        ...
        for satellite in pre_region:  # satellite_folder: 2_250_0.png
            ...
            if matching_score < 0.4:
                # reverse tensorB
                tensorB_reverse = tensorB.flip(1,2,3)
                pred_reverse = match_pair(extractor, matcher, tensorA, tensorB_reverse)
                kpts0_reverse, kpts1_reverse, matches_reverse = pred_reverse['keypoints0'], pred_reverse['keypoints1'], pred_reverse['matches']
                m_kpts0_reverse, m_kpts1_reverse = kpts0_reverse[matches_reverse[..., 0]], kpts1_reverse[matches_reverse[..., 1]]
                if(len(m_kpts0_reverse) < 4):
                    continue
                H_reverse, _ = cv2.findHomography(m_kpts0_reverse.numpy(), m_kpts1_reverse.numpy(), cv2.RANSAC, 5.0)
                matching_score_reverse = pred_reverse['matching_scores'].numpy().mean()
            
            if max(matching_score, matching_score_reverse) > best_score:
                best_score = max(matching_score, matching_score_reverse)
                best_H = H if matching_score > matching_score_reverse else H_reverse
                best_sub = satellite
                best_match = len(matches) if matching_score > matching_score_reverse else len(matches_reverse)
                if matching_score >= matching_score_reverse:
                    best_x, best_y = coords(transformation(pt_drone.T, best_H))
                else:
                    best_x, best_y = 500 - coords(transformation(pt_drone.T, H_reverse))[0], 500 - coords(transformation(pt_drone.T, H_reverse))[1]
            # if len(matches) > best_match:
            #     best_match = len(matches)
            #     best_score = matching_score
            #     best_sub = satellite
        ...
        resX, resY = float(x_gt), float(y_gt)
        end = time.time()
